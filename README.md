...
按照 `A股交易辅助决策工具_PRD_V1.1.md` 实现的个人日线研究工作台，支持独立的全市场实时报价快照。系统完成“同步数据 → 查看市场 → 组合筛选 → 查看个股 → 加入自选 → 处理提醒 → 生成报告”闭环，不接实盘、不自动下单。
...
行情看板顶部点击「刷新实时行情」，后台读取新浪沪深京完整股票池并分组获取腾讯报价。无需先同步日线；页面显示采集进度，完成后展示最新价、涨跌幅、成交量（股）、成交额（万元）和每只股票的来源报价时间（北京时间），支持代码/名称搜索及分页。

- 一次点击获取一次快照，不会持续自动采集；刷新期间重复请求复用任务，结束后有 30 秒冷却。
- 全市场分批抓取，不保证所有股票处于同一瞬间，也不保证行情无延迟。抓取完成时间与来源报价时间分别显示；休市、停牌或旧报价不能视为当前成交。
- 完整率低于 99% 或股票池失败时保留上次快照；达到 99% 但有缺失时明确显示缺失股票。市场统计只包括抓取当日已取得的有效报价，不补齐或推算缺失报价。
- 实时快照持久化保存最新一次成功结果，服务重启会把中断任务标记为失败，允许重试。
- 实时数据不写入 `data_batch`、`daily_price` 或指标表，不改变筛选、提醒和报告。下方原有指数与日线看板仍按其交易日展示。

自选股页面也提供「刷新自选股行情」：只查询点击时已经加入自选的股票，不拉取全市场股票池。自选与全市场各自保存刷新任务、冷却时间和快照，互不覆盖；采集中新增的自选股需要下一次点击才会纳入。列表优先显示自选实时报价及来源时间；尚无报价的股票明确显示「日线参考」。信号、风险和提醒继续使用标明日期的收盘日线。空自选禁用刷新，失败保留上次快照。
...
- 指标分析只处理已收盘交易日的日线；实时报价通过独立按钮手动获取快照，不提供持续推送盘口。日线自动同步默认于上海时间交易日 18:30 运行。
