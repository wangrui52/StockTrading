...
export function SettingsPage() {
  ...
  const [ruleName, setRuleName] = useState('自定义 RSI 过热')
  const [ruleCode, setRuleCode] = useState('CUSTOM_RSI')
  useEffect(() => {
  ...
  const createRule = useMutation({ mutationFn: () => request('/alert-rules', { method: 'POST', body: JSON.stringify({ name: ruleName, rule_code: ruleCode, threshold, enabled: true }) }), onSuccess: () => client.invalidateQueries({ queryKey: ['alert-rules'] }) })
  const toggleRule = useMutation({ mutationFn: ({ logicalId, threshold: value, enabled: valueEnabled }: { logicalId: number; threshold: number; enabled: boolean }) => request(`/alert-rules/${logicalId}`, { method: 'PATCH', body: JSON.stringify({ threshold: value, enabled: valueEnabled }) }), onSuccess: () => client.invalidateQueries({ queryKey: ['alert-rules'] }) })
  const deleteRule = useMutation({ mutationFn: (logicalId: number) => request(`/alert-rules/${logicalId}`, { method: 'DELETE' }), onSuccess: () => client.invalidateQueries({ queryKey: ['alert-rules'] }) })
  ...
  return <div className="page-stack"><section className="panel"><p className="eyebrow">运行状态</p><h2>系统设置</h2><div className="context-strip"><span>数据源 {settings.data.adapter_version}</span><span>当前规则 {settings.data.current_rule_version}</span><span>最近成功批次 {settings.data.last_successful_batch ?? '--'}</span><span>完整率 {settings.data.completeness_rate == null ? '--' : `${(settings.data.completeness_rate * 100).toFixed(1)}%`}</span></div>{settings.data.failed_jobs.length ? settings.data.failed_jobs.map((job) => <p role="alert" key={String(job.id)}>失败任务 #{String(job.id)} · {String(job.stage)} · {String(job.error_summary)}</p>) : <p className="muted">最近没有失败任务。</p>}</section><section className="panel"><h3>自动同步</h3><form className="inline-form" onSubmit={scheduleSubmit}><label><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} /> 启用交易日自动同步</label><label>执行时间<input type="time" value={time} onChange={(event) => setTime(event.target.value)} /></label><button type="submit">保存同步设置</button></form>{saveSchedule.isSuccess && <p>同步设置已保存。</p>}</section><section className="panel"><h3>指标规则版本</h3><p className="muted">修改参数会创建新版本，不改写历史信号。</p><form className="inline-form" onSubmit={versionSubmit}><label>RSI 周期<input type="number" min="2" value={rsiPeriod} onChange={(event) => setRsiPeriod(Number(event.target.value))} /></label><label><input type="checkbox" checked={confirm} onChange={(event) => setConfirm(event.target.checked)} /> 确认触发重算</label><button type="submit">创建规则版本</button></form>{createVersion.isError && <p role="alert">必须确认重算后才能修改。</p>}{createVersion.isSuccess && <p>新规则版本已创建，等待重算。</p>}</section><section className="panel"><h3>自定义提醒规则</h3><form className="inline-form" onSubmit={alertSubmit}><label>规则名称<input value={ruleName} onChange={(event) => setRuleName(event.target.value)} /></label><label>规则代码<input value={ruleCode} onChange={(event) => setRuleCode(event.target.value)} /></label><label>阈值<input type="number" value={threshold} onChange={(event) => setThreshold(Number(event.target.value))} /></label><button type="submit">新增规则</button></form>{rules.data?.items.map((item) => <div className="alert-row" key={item.id}><span>{item.name} · 阈值 {item.threshold} · v{item.version} · {item.enabled ? '启用' : '停用'}</span><div className="inline-form"><button type="button" onClick={() => toggleRule.mutate({ logicalId: item.logical_id, threshold: item.threshold, enabled: !item.enabled })}>{item.enabled ? '停用' : '启用'}</button><button type="button" onClick={() => deleteRule.mutate(item.logical_id)}>删除</button></div></div>)}</section></div>
}
