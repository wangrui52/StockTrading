...
def upgrade() -> None:
    op.add_column(
        "data_batch",
        sa.Column("risk_acknowledged", sa.Boolean(), nullable=False, server_default="0"),
    )
    op.add_column(
    ...
    op.add_column(
        "sync_job", sa.Column("failed_items", sa.JSON(), nullable=False, server_default="[]")
    )
    op.add_column(
...
def downgrade() -> None:
    op.drop_column("daily_price", "fetched_at")
    ...
    op.drop_column("sync_job", "failed_items")
    ...
    op.drop_column("data_batch", "risk_acknowledged")
