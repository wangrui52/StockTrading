# Task Index

| Task | 状态 | 对应 S/F | 约束 | 证据 | 风险 |
|---|---|---|---|---|---|
| T001 工程与测试基线 | done | S1,S3,S4,F1 | C001,C002 | `evidence/T001-engineering-baseline.md` | FastAPI TestClient 第三方弃用警告 |
| T002 指标引擎 | done | S2,S4,F2,F4 | C002 | `evidence/T002-indicator-engine.md` | 金样本后续扩展 |
| T003 信号引擎 | done | S2,S4,F2,F4 | C002 | `evidence/T003-signal-engine.md` | 真实数据回放待后续 |
| T004 数据库与 repository | done | S2,S4,S5,F3 | C001,C003 | `evidence/T004-database-repositories.md` | 并发报告版本待 PostgreSQL 复核 |
| T005 数据 adapter 与同步 | done | S1,S4,F3,F6 | C002,C003 | `evidence/T005-sync-pipeline.md` | 全市场性能待压测 |
| T006 P0 REST interface | in_progress | S1,S2,S3 | C001 | pending | 契约完整性 |
| T007 前端 P0 页面 | pending | S1,S3,S6,F1 | C001,C002 | pending | 用户体验 |
| T008 P1 功能 | pending | S1,S2,S6 | C001,C002 | pending | 范围较大 |
| T009 E2E 本地闭环 | pending | S1,S4,S6,F1,F4 | C002,C003 | pending | 浏览器环境 |
| T010 部署与最终审计 | pending | S3,S5 | C001,C003,C005 | pending | 容器环境 |
